...
function outed(meet, boss){
  const sum = 0;
  const array = Object.entries(meet);
  const points = array.reduce(((acc, curr) => acc += curr[1]), 0);
  const bossPoints = array.find((current) => current[0] === boss);
  const happiness = (points + bossPoints[1])/array.length;
  if (happiness <= 5) {
    return 'Get Out Now!';
  } else {
